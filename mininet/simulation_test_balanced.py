...
import sys
import scalefree
import errors_balanced as errors
import ConfigParser
import time
import logging
import json
...
class Simulation():
	...
	def run(self, predefined_error = 0):
		...
		while (now_timestamp - orig_timestamp).total_seconds() < minutes*60:
			...
			if predefined_error != 0:
				...
				# Checking error 3 ...
				self.create_error(random.randint(4,13), nm_ho, datac, sim_id, logger, error_interval)
				'''
	...
	def create_error(self, err, nm_ho, datac, sim_id, logger, error_interval = 10):
		...
		if err == 1:
			...
			print 'Ok, none errors'
			errors.send_report('ok', {'Timestamp': str(datetime.now())}, sim_id, logger)
			time.sleep(error_interval)
